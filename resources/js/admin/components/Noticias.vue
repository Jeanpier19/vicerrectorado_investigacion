        ...
        <div class="row">
          ...
          <div class="col-lg-8 col-sm-12">
            <div class="input-group mb-2">
              ...
              <button type="button" class="btn btn-success ml-1" @click.prevent="openModalCreate()"><i class="fas fa-fw fa-plus-circle"></i>&nbsp;Registrar Noticia</button>
            </div>
          </div>
        </div>
                ...
                <td>
                  <ul class="list-unstyled">
                    ...
                    <li v-for="galeria in item.images" class="text-center mb-2">
                      <a :href="getItemImage(galeria)" class="fancybox" :data-fancybox="'Noticia' + item.id" :data-caption="item.titulo">
            ...
            <form @submit.prevent="editMode ? update() : create()">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'>
                  <span v-if="editMode"><i class='fas fa-fw fa-edit'></i>&nbsp;Editar Noticia</span>
                  <span v-else><i class='fas fa-fw fa-plus-circle'></i>&nbsp;Registrar Noticia</span>
                </h5>
              ...
              <div class="modal-body">
                <div class="form-row">
                  <div class="form-group col-xl-5 col-lg-5 col-md-5 col-sm-12">
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <form method="post" @submit.prevent="addGaleria(form.id)">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'><i class='fas fa-fw fa-plus-circle'></i> Añadir Imágenes a la Noticia: {{ form.titulo }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <form method="post" @submit.prevent="addDocumento(form.id)">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'><i class='fas fa-fw fa-plus-circle'></i> Añadir un Documento Anexo a la Noticia: {{ form.titulo }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getItems(page) {
        this.loader = true;
        axios.get('/api/noticia?page=' + page + '&search=' + this.search).then(response => {
      ...
      getItemImage(file) {
        let filePath = null;
        if(file != null && file != '') {
          filePath = "/archivos/imagenes/noticias/" + file.path;
        }else{
          filePath = '';
        }
        return filePath;
      ...
      getItemDocument(file) {
        let filePath = null;
        if(file != null && file != '') {
          filePath = "/archivos/documentos/noticias/" + file.path;
        }else{
          filePath = '';
        }
        return filePath;
      ...
      create() {
        this.isLoading = true;
        this.form.post('/api/noticia').then(response => {
      ...
      update(id) {
        this.isLoading = true;
        this.form.put('/api/noticia/' + this.form.id).then(response => {
          if(response.data.result) {
            Fire.$emit('transaction');
            $('#modal').modal('hide');
            toastr.success(response.data.message, '¡Completado!');
      ...
      disableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/noticia/alta-baja/0/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      enableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/noticia/alta-baja/1/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      deleteItem(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            this.form.delete('/api/noticia/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      addGaleria: function(id) {
        ...
        this.isLoading = true;
        let url = '/api/noticia/add-galeria';
      ...
      deleteGaleria: function(item, noticia) {
        let img = "/archivos/imagenes/noticias/" + item.path;
        Swal.fire({
          title: '¿Desea eliminar la siguiente imagen de la noticia?',
          icon: 'warning',
          ...
          if(result.value) {
            this.isLoading = true;
            let url = '/api/noticia/delete-galeria';
            let data = new FormData();
            data.append('image_id', item.id);
            data.append('noticia_id', noticia.id);
      ...
      addDocumento: function(id) {
        ...
        this.isLoading = true;
        let url = '/api/noticia/add-documento';
        let data = new FormData();
      ...
      deleteDocumento: function(item, noticia) {
        Swal.fire({
          title: '¿Desea eliminar el siguiente documento de la Noticia?',
          icon: 'warning',
          ...
          if (result.value) {
            let url = '/api/noticia/delete-documento';
            let data = new FormData();
