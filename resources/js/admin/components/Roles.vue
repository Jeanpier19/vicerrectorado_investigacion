          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="item, key in items">
                ...
                <td>
                  <ul class="list-group">
                    <li v-for="permission, key in item.permissions" class="list-group-item" v-text="permission.name"></li>
                  </ul>
                  <span v-if="item.permissions.length==0">Ninguno</span>
                </td>
                <td>
          ...
          <div class="modal-content">
            <form @submit.prevent="editMode ? update() : create()">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'><span v-if="editMode"><i class='fas fa-fw fa-edit'></i>&nbsp;Editar Rol</span><span v-else><i class='fas fa-fw fa-plus-circle'></i>&nbsp;Registrar Rol</span></h5>
              ...
              <div class="modal-body">
                ...
                <div class="form-group">
                  <label class="font-weight-bold">Lista de permisos</label>
                  <ul class="list-unstyled m-0">
                    <li v-for="permission, key in permissions">
                      <label>
                        <input type="checkbox" :id="permission.id" :value="permission.id" v-model="form.permissions">
                        <label :for="permission.id" v-text="permission.name"></label>
                      </label>
                    </li>
                  </ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      getItems(page) {
        this.loader = true;
        axios.get('/api/role?page=' + page + '&search=' + this.search).then(response => {
      ...
      deleteItem(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            this.form.delete('/api/role/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
