        ...
        <div class="row">
          ...
          <div class="col-lg-8 col-sm-12">
            <div class="input-group mb-2">
              ...
              <button type="button" class="btn btn-success ml-1" @click.prevent="openModalCreate()"><i class="fas fa-fw fa-plus-circle"></i>&nbsp;Registrar Banner</button>
            </div>
          </div>
        </div>
          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="item, key in items">
                ...
                <td>
                  <a :href="item.link" class="font-weight-bold" target="_blank" v-if="item.link">Click aquí</a>
                  <span v-else>Ninguno</span>
                </td>
                <td>
                  <a :href="getItemImage(item.image)" class="fancybox" data-fancybox="Banner">
            ...
            <form @submit.prevent="editMode ? update() : create()">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'>
                  <span v-if="editMode"><i class='fas fa-fw fa-edit'></i>&nbsp;Editar Banner</span>
                  <span v-else><i class='fas fa-fw fa-plus-circle'></i>&nbsp;Registrar Banner</span>
                </h5>
              ...
              <div class="modal-body">
                <div class="form-row">
                  <div class="form-group col-xl-8 col-lg-8 col-md-6 col-sm-12">
                    ...
                    <!-- udpate -->
                    <small v-show="errors.link" v-for="error in errors.link" class="text-danger font-weight-bold" v-text="error + '. '"></small>
                  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getItems(page) {
        this.loader = true;
        axios.get('/api/banner?page=' + page + '&search=' + this.search).then(response => {
      ...
      getItemImage(image) {
        let imagePath = null;
        if(image != null && image != '') {
          imagePath = "/archivos/imagenes/banners/" + image.path;
        }else {
           imagePath = '';
        }
        return imagePath;
      ...
      openModalCreate() {
        ...
        $('#modal').on('shown.bs.modal', function() { $('#link').focus() });
      },
      ...
      create() {
        this.isLoading = true;
        this.form.submit('post', '/api/banner', {
      ...
      update() {
        this.isLoading = true;
        let url = '/api/banner/' + this.form.id;
      ...
      disableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/banner/alta-baja/0/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      enableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/banner/alta-baja/1/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      deleteItem(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            this.form.delete('/api/banner/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
