        ...
        <div class="row">
          ...
          <div class="col-lg-8 col-sm-12">
            <div class="input-group mb-2">
              ...
              <button type="button" class="btn btn-success ml-1" @click.prevent="openModalCreate()"><i class="fas fa-fw fa-plus-circle"></i>&nbsp;Registrar Convocatoria</button>
            </div>
          </div>
        </div>
          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="item, key in items">
                ...
                <td v-text="item.tipo_convocatoria.nombre"></td>
                <td v-text="item.titulo"></td>
                <td>{{ item.desde | myDate }}</td>
                <td v-if="item.hasta">{{ item.hasta | myDate }}</td>
                <td v-else>---</td>
                <td>
                  <ul class="list-unstyled">
                    ...
                    <li v-for="galeria in item.images" class="text-center mb-2">
                      <a :href="getItemImage(galeria)" class="fancybox" :data-fancybox="'Convocatoria' + item.id" :data-caption="item.titulo">
            ...
            <form @submit.prevent="editMode ? update() : create()">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'>
                  <span v-if="editMode"><i class='fas fa-fw fa-edit'></i>&nbsp;Editar Convocatoria</span>
                  <span v-else><i class='fas fa-fw fa-plus-circle'></i>&nbsp;Registrar Convocatoria</span>
                </h5>
              ...
              <div class="modal-body">
                <div class="form-row">
                  <div class="form-group col-md-3">
                    <label>Tipo de Convocatoria&nbsp;<span class="text-danger">*</span></label>
                    <select v-model="form.tipo_convocatoria_id" type="text" name="tipo_convocatoria_id" id="tipo_convocatoria_id"
                    class="form-control" :class="{ 'is-invalid': form.errors.has('tipo_convocatoria_id') }">
                      <option value="" disabled selected>Selecione el Tipo de Convocatoria...</option>
                      <option v-for="item in tipos_convocatoria" :value="item.id" v-text="item.nombre"></option>
                    </select>
                    <has-error :form="form" field="tipo_convocatoria_id"></has-error>
                  </div>
                  <div class="form-group col-md-3">
                    <label>Desde&nbsp;<span class="text-danger">*</span></label>
                    <input v-model="form.desde" type="date" name="desde" id="desde" 
                    class="form-control" :class="{ 'is-invalid': form.errors.has('desde') }">
                    <has-error :form="form" field="desde"></has-error>
                  </div>
                  <div class="form-group col-md-3">
                    <label>Hasta</label>
                    <input v-model="form.hasta" type="date" name="hasta" id="hasta" 
                    class="form-control" :class="{ 'is-invalid': form.errors.has('hasta') }">
                    <has-error :form="form" field="hasta"></has-error>
                  </div>
                  <div class="form-group col-md-3">
                ...
                <div class="form-group">
                  <label>Agenda</label>
                  <ckeditor :editor="editor" v-model="form.agenda" :config="editorConfig" :class="{ 'is-invalid': form.errors.has('agenda') }"></ckeditor>
                  <has-error :form="form" field="agenda"></has-error>
                </div>
              </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <form method="post" @submit.prevent="addGaleria(form.id)">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'><i class='fas fa-fw fa-plus-circle'></i> Añadir Imágenes a la Convocatoria: {{ form.titulo }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <form method="post" @submit.prevent="addDocumento(form.id)">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'><i class='fas fa-fw fa-plus-circle'></i> Añadir un Documento Anexo a la Convocatoria: {{ form.titulo }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getItems(page) {
        this.loader = true;
        axios.get('/api/convocatoria?page=' + page + '&search=' + this.search).then(response => {
          this.items = response.data.items.data;
          this.pagination = response.data.pagination;
          this.tipos_convocatoria = response.data.tipos_convocatoria;
          this.loader = false;
        }).catch(() => {
      ...
      getItemImage(file) {
        let filePath = null;
        if(file != null && file != '') {
          filePath = "/archivos/imagenes/convocatorias/" + file.path;
        }else{
          filePath = '';
        }
        return filePath;
      ...
      getItemDocument(file) {
        let filePath = null;
        if(file != null && file != '') {
          filePath = "/archivos/documentos/convocatorias/" + file.path;
        }else{
          filePath = '';
        }
        return filePath;
      ...
      openModalCreate() {
        ...
        $('#modal').on('shown.bs.modal', function() { $('#tipo_convocatoria_id').focus() });
      },
      openModalUpdate(item) {
        this.editMode = true;
        let dataToFill = {
          'id': item.id,
          'tipo_convocatoria_id': item.tipo_convocatoria_id,
          'titulo': item.titulo,
      ...
      create() {
        this.isLoading = true;
        this.form.post('/api/convocatoria').then(response => {
      ...
      update(id) {
        this.isLoading = true;
        this.form.put('/api/convocatoria/' + this.form.id).then(response => {
          if(response.data.result) {
            Fire.$emit('transaction');
            $('#modal').modal('hide');
            toastr.success(response.data.message, '¡Completado!');
      ...
      disableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/convocatoria/alta-baja/0/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      enableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/convocatoria/alta-baja/1/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      deleteItem(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            this.form.delete('/api/convocatoria/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      addGaleria: function(id) {
        ...
        this.isLoading = true;
        let url = '/api/convocatoria/add-galeria';
      ...
      deleteGaleria: function(item, convocatoria) {
        let img = "/archivos/imagenes/convocatorias/" + item.path;
        Swal.fire({
          title: '¿Desea eliminar la siguiente imagen de la convocatoria?',
          icon: 'warning',
          ...
          if(result.value) {
            this.isLoading = true;
            let url = '/api/convocatoria/delete-galeria';
            let data = new FormData();
            data.append('image_id', item.id);
            data.append('convocatoria_id', convocatoria.id);
      ...
      addDocumento: function(id) {
        ...
        this.isLoading = true;
        let url = '/api/convocatoria/add-documento';
        let data = new FormData();
      ...
      deleteDocumento: function(item, convocatoria) {
        Swal.fire({
          title: '¿Desea eliminar el siguiente documento de la Convocatoria?',
          icon: 'warning',
          ...
          if (result.value) {
            let url = '/api/convocatoria/delete-documento';
            let data = new FormData();
