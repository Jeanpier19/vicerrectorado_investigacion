  ...
  <div class="container-fluid pt-3">
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Tipos de Convenio Específico</li>
      </ol>
    </nav>
    <div class="card">
      <div class="card-body">
        <div class="row">
          ...
          <div class="col-lg-8 col-sm-12">
            <div class="input-group mb-2">
              ...
              <button type="button" class="btn btn-success ml-1" @click.prevent="openModalCreate()"><i class="fas fa-fw fa-plus-circle"></i>&nbsp;Registrar Tipo de Convenio Específico</button>
            </div>
          </div>
        </div>
          ...
          <table class="table table-striped">
            <thead>
              <tr>
                <th>#</th>
                <th>Nombre</th>
                <th>Grupo de Convenio Específico</th>
                <th>Prioridad</th>
                <th>Activo</th>
            ...
            <tbody>
              <tr v-for="item, key in items">
                ...
                <td v-text="item.grupo_convenio_especifico.nombre"></td>
                <td>
            ...
            <form @submit.prevent="editMode ? update() : create()">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'>
                  <span v-if="editMode"><i class='fas fa-fw fa-edit'></i>&nbsp;Editar Tipo de Convenio Específico</span>
                  <span v-else><i class='fas fa-fw fa-plus-circle'></i>&nbsp;Registrar Tipo de Convenio Específico</span>
                </h5>
              ...
              <div class="modal-body">
                <div class="form-row">
                  ...
                  <div class="form-group col-md-5">
                    <label>Grupo de Convenio Específico&nbsp;<span class="text-danger">*</span></label>
                    <select v-model="form.grupo_convenio_especifico_id" type="text" name="grupo_convenio_especifico_id" id="grupo_convenio_especifico_id"
                    class="form-control" :class="{ 'is-invalid': form.errors.has('grupo_convenio_especifico_id') }">
                      <option value="" disabled selected>Selecione el Grupo de Convenio Específico...</option>
                      <option v-for="item in grupos_convenio_especifico" :value="item.id" v-text="item.nombre"></option>
                    </select>
                    <has-error :form="form" field="grupo_convenio_especifico_id"></has-error>
                  </div>
  ...
  export default {
    data() {
      return {
        ...
        editMode: false,
        items: {},
        grupos_convenio_especifico: {},
        form: new Form({
          'id': '',
          'nombre': '',
          'grupo_convenio_especifico_id': '',
          'main': 0,
        }),
        pagination: {
          'total': 0,
    ...
    methods: {
      getItems(page) {
        this.loader = true;
        axios.get('/api/tipo-convenio-especifico?page=' + page + '&search=' + this.search).then(response => {
          this.items = response.data.items.data;
          this.pagination = response.data.pagination;
          this.grupos_convenio_especifico = response.data.grupos_convenio_especifico;
          this.loader = false;
        }).catch(() => {
      ...
      openModalUpdate(item) {
        ...
        let dataToFill = {
          'id': item.id,
          'nombre': item.nombre,
          'grupo_convenio_especifico_id': item.grupo_convenio_especifico_id,
          'main': item.main,
        };
        this.form.reset();
        this.form.clear();
      ...
      create() {
        this.isLoading = true;
        this.form.post('/api/tipo-convenio-especifico').then(response => {
      ...
      update(id) {
        this.isLoading = true;
        this.form.put('/api/tipo-convenio-especifico/' + this.form.id).then(response => {
          if(response.data.result) {
            Fire.$emit('transaction');
            $('#modal').modal('hide');
            toastr.success(response.data.message, '¡Completado!');
      ...
      disableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/tipo-convenio-especifico/alta-baja/0/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      enableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/tipo-convenio-especifico/alta-baja/1/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      deleteItem(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            this.form.delete('/api/tipo-convenio-especifico/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
