  ...
  <div class="container-fluid pt-3">
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Tipos de Institución</li>
      </ol>
    </nav>
    <div class="card">
      <div class="card-body">
        <div class="row">
          ...
          <div class="col-lg-8 col-sm-12">
            <div class="input-group mb-2">
              ...
              <button type="button" class="btn btn-success ml-1" @click.prevent="openModalCreate()"><i class="fas fa-fw fa-plus-circle"></i>&nbsp;Registrar Tipo de Institución</button>
            </div>
          </div>
        </div>
            ...
            <form @submit.prevent="editMode ? update() : create()">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'>
                  <span v-if="editMode"><i class='fas fa-fw fa-edit'></i>&nbsp;Editar Tipo de Institución</span>
                  <span v-else><i class='fas fa-fw fa-plus-circle'></i>&nbsp;Registrar Tipo de Institución</span>
                </h5>
...
<script>
  ...
  export default {
    ...
    methods: {
      getItems(page) {
        this.loader = true;
        axios.get('/api/tipo-institucion?page=' + page + '&search=' + this.search).then(response => {
      ...
      create() {
        this.isLoading = true;
        this.form.post('/api/tipo-institucion').then(response => {
      ...
      update(id) {
        this.isLoading = true;
        this.form.put('/api/tipo-institucion/' + this.form.id).then(response => {
          if(response.data.result) {
            Fire.$emit('transaction');
            $('#modal').modal('hide');
            toastr.success(response.data.message, '¡Completado!');
      ...
      disableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/tipo-institucion/alta-baja/0/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      enableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/tipo-institucion/alta-baja/1/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      deleteItem(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            this.form.delete('/api/tipo-institucion/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
