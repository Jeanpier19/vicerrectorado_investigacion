        ...
        <div class="row">
          ...
          <div class="col-lg-8 col-sm-12">
            <div class="input-group mb-2">
              ...
              <button type="button" class="btn btn-success ml-1" @click.prevent="openModalCreate()"><i class="fas fa-fw fa-plus-circle"></i>&nbsp;Registrar Semestre</button>
            </div>
          </div>
        </div>
            ...
            <form @submit.prevent="editMode ? update() : create()">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'>
                  <span v-if="editMode"><i class='fas fa-fw fa-edit'></i>&nbsp;Editar Semestre</span>
                  <span v-else><i class='fas fa-fw fa-plus-circle'></i>&nbsp;Registrar Semestre</span>
                </h5>
              ...
              <div class="modal-body">
                <div class="form-row">
                  ...
                  <div class="form-group col-md-5">
                    ...
                    <input v-model="form.periodo" type="text" name="periodo" id="periodo"
                ...
                <div class="form-group m-0">
                  <input v-model="form.slug" type="hidden" name="slug" id="slug"
...
<script>
  ...
  export default {
    ...
    methods: {
      getItems(page) {
        this.loader = true;
        axios.get('/api/semestre?page=' + page + '&search=' + this.search).then(response => {
      ...
      openModalCreate() {
        ...
        $('#modal').on('shown.bs.modal', function() { $('#anio').focus() });
      },
      ...
      create() {
        this.form.slug = this.form.anio + '-' + this.form.periodo;
        this.isLoading = true;
        this.form.post('/api/semestre').then(response => {
          ...
          if(error.response.status == 422) {
            toastr.error('Por favor verifique los campos obligatorios antes de guardar 2', '¡Error!');
          }else {
            toastr.error('Hubo un error inesperado, recargue la página e intente nuevamente 2', '¡Error!');
          }
          this.isLoading = false;
        });
      },
      update(id) {
        this.form.slug = this.form.anio + '-' + this.form.periodo;
        this.isLoading = true;
        this.form.put('/api/semestre/' + this.form.id).then(response => {
          if(response.data.result) {
            Fire.$emit('transaction');
            $('#modal').modal('hide');
            toastr.success(response.data.message, '¡Completado!');
      ...
      disableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/semestre/alta-baja/0/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      enableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/semestre/alta-baja/1/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      deleteItem(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            this.form.delete('/api/semestre/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
