        ...
        <div class="row">
          ...
          <div class="col-lg-8 col-sm-12">
            <div class="input-group mb-2">
              ...
              <button type="button" class="btn btn-success ml-1" @click.prevent="openModalCreate()"><i class="fas fa-fw fa-plus-circle"></i>&nbsp;Registrar Galería</button>
            </div>
          </div>
        </div>
                ...
                <td>
                  <ul class="list-unstyled">
                    ...
                    <li v-for="galeria in item.images" class="text-center mb-2">
                      <a :href="getItemImage(galeria)" class="fancybox" :data-fancybox="'Galería' + item.id" :data-caption="item.titulo">
            ...
            <form @submit.prevent="editMode ? update() : create()">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'>
                  <span v-if="editMode"><i class='fas fa-fw fa-edit'></i>&nbsp;Editar Galería</span>
                  <span v-else><i class='fas fa-fw fa-plus-circle'></i>&nbsp;Registrar Galería</span>
                </h5>
              ...
              <div class="modal-body">
                ...
                <div class="form-group">
                  ...
                  <textarea v-model="form.descripcion" type="text" name="descripcion" rows="4" 
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <form method="post" @submit.prevent="addGaleria(form.id)">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'><i class='fas fa-fw fa-plus-circle'></i> Añadir Imágenes a la Galería: {{ form.titulo }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getItems(page) {
        this.loader = true;
        axios.get('/api/galeria?page=' + page + '&search=' + this.search).then(response => {
      ...
      getItemImage(file) {
        let filePath = null;
        if(file != null && file != '') {
          filePath = "/archivos/imagenes/galerias/" + file.path;
        }else{
          filePath = '';
        }
        return filePath;
      ...
      getItemDocument(file) {
        let filePath = null;
        if(file != null && file != '') {
          filePath = "/archivos/documentos/galerias/" + file.path;
        }else{
          filePath = '';
        }
        return filePath;
      ...
      create() {
        this.isLoading = true;
        this.form.post('/api/galeria').then(response => {
      ...
      update(id) {
        this.isLoading = true;
        this.form.put('/api/galeria/' + this.form.id).then(response => {
          if(response.data.result) {
            Fire.$emit('transaction');
            $('#modal').modal('hide');
            toastr.success(response.data.message, '¡Completado!');
      ...
      disableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/galeria/alta-baja/0/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      enableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/galeria/alta-baja/1/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      deleteItem(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            this.form.delete('/api/galeria/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      addGaleria: function(id) {
        ...
        this.isLoading = true;
        let url = '/api/galeria/add-galeria';
      ...
      deleteGaleria: function(item, galeria) {
        let img = "/archivos/imagenes/galerias/" + item.path;
        Swal.fire({
          title: '¿Desea eliminar la siguiente imagen de la galeria?',
          icon: 'warning',
          ...
          if(result.value) {
            this.isLoading = true;
            let url = '/api/galeria/delete-galeria';
            let data = new FormData();
            data.append('image_id', item.id);
            data.append('galeria_id', galeria.id);
      ...
      addDocumento: function(id) {
        ...
        this.isLoading = true;
        let url = '/api/galeria/add-documento';
        let data = new FormData();
      ...
      deleteDocumento: function(item, galeria) {
        Swal.fire({
          title: '¿Desea eliminar el siguiente documento de la Galería?',
          icon: 'warning',
          ...
          if (result.value) {
            let url = '/api/galeria/delete-documento';
            let data = new FormData();
