  ...
  <div class="container-fluid pt-3">
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Proyectos de Investigación</li>
      </ol>
    </nav>
    <div class="card">
      <div class="card-body">
        <div class="row">
          ...
          <div class="col-lg-8 col-sm-12">
            <div class="input-group mb-2">
              ...
              <button type="button" class="btn btn-success ml-1" @click.prevent="openModalCreate()"><i class="fas fa-fw fa-plus-circle"></i>&nbsp;Registrar Proyecto de Investigación</button>
            </div>
          </div>
        </div>
            ...
            <form @submit.prevent="editMode ? update() : create()">
              <div class="modal-header">
                <h5 class='modal-title font-weight-bold'>
                  <span v-if="editMode"><i class='fas fa-fw fa-edit'></i>&nbsp;Editar Proyecto de Investigación</span>
                  <span v-else><i class='fas fa-fw fa-plus-circle'></i>&nbsp;Registrar Proyecto de Investigación</span>
                </h5>
...
<script>
  ...
  export default {
    ...
    methods: {
      getItems(page) {
        this.loader = true;
        axios.get('/api/proyecto-investigacion?page=' + page + '&search=' + this.search).then(response => {
      ...
      getLineas() {
        ...
        this.isLoading = true;
        let url = '/api/proyecto-investigacion/get-lineas?area_id=' + this.form.area_id;
      ...
      getSublineas() {
        ...
        this.form.sublinea_id = '';
        this.isLoading = true;
        let url = '/api/proyecto-investigacion/get-sublineas?linea_id=' + this.form.linea_id;
        axios.post(url).then(response => {
      ...
      getItemDocument(file) {
        let filePath = null;
        if(file != null && file != '') {
          filePath = "/archivos/documentos/proyecto-investigacions/" + file.path;
        }else{
          filePath = '';
        }
        return filePath;
      ...
      create() {
        this.isLoading = true;
        this.form.post('/api/proyecto-investigacion').then(response => {
      ...
      update() {
        this.isLoading = true;
        this.form.put('/api/proyecto-investigacion/' + this.form.id).then(response => {
          if(response.data.result) {
            Fire.$emit('transaction');
            $('#modal').modal('hide');
            toastr.success(response.data.message, '¡Completado!');
      ...
      disableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/proyecto-investigacion/alta-baja/0/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      enableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/proyecto-investigacion/alta-baja/1/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      deleteItem(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            this.form.delete('/api/proyecto-investigacion/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
