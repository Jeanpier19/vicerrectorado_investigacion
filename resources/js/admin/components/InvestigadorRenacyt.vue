...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getItems(page) {
        this.loader = true;
        axios.get('/api/investigador-renacyt?page=' + page + '&search=' + this.search).then(response => {
      ...
      searchUser(search, loading) {
        loading(true);
        let url = "/api/investigador-renacyt/search/user?search=" + search;
        axios.get(url).then(response => {
      ...
      create() {
        this.isLoading = true;
        this.form.submit('post', '/api/investigador-renacyt', {
      ...
      update() {
        this.isLoading = true;
        let url = '/api/investigador-renacyt/' + this.form.id;
      ...
      disableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/investigador-renacyt/alta-baja/0/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      enableItem: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/investigador-renacyt/alta-baja/1/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      deleteItem(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            this.form.delete('/api/investigador-renacyt/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction');
                $('#modal').modal('hide');
                toastr.success(response.data.message, '¡Completado!');
      ...
      passwordReset: function(id) {
        Swal.fire({
          ...
          if(result.value) {
            this.isLoading = true;
            axios.post('/api/investigador-renacyt/restablecer-password/' + id).then(response => {
              if(response.data.result) {
                Fire.$emit('transaction')
                toastr.success(response.data.message, '¡Completado!');
                Swal.fire({
